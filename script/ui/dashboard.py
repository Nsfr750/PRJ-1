...
class DashboardDialog(QDialog):
    ...
    def __init__(self, scanner: ProjectScanner, parent=None):
        super().__init__(parent)
        self.scanner = scanner
        self.setWindowTitle("Project Statistics Dashboard")
    ...
    def setup_ui(self):
        ...
        title_label = QLabel("Project Statistics Dashboard")
    ...
    def create_overview_tab(self):
        ...
        # Total projects card
        total_card = self.create_stat_card("Total Projects", "0", "projects")
        cards_layout.addWidget(total_card, 0, 0)
        
        # Languages card
        languages_card = self.create_stat_card("Languages", "0", "languages")
        cards_layout.addWidget(languages_card, 0, 1)
        
        # Categories card
        categories_card = self.create_stat_card("Categories", "0", "categories")
        cards_layout.addWidget(categories_card, 0, 2)
        
        # Favorites card
        favorites_card = self.create_stat_card("Favorites", "0", "projects")
        cards_layout.addWidget(favorites_card, 1, 0)
        
        # Recent projects card
        recent_card = self.create_stat_card("Recent Projects", "0", "projects")
        cards_layout.addWidget(recent_card, 1, 1)
        
        # Total size card
        size_card = self.create_stat_card("Total Size", "0 MB", "disk usage")
        ...
        self.language_chart = self.create_pie_chart("Language Distribution")
        charts_splitter.addWidget(self.language_chart)
        
        # Category distribution chart
        self.category_chart = self.create_pie_chart("Category Distribution")
    ...
    def create_language_tab(self):
        ...
        # Language chart
        self.language_bar_chart = self.create_bar_chart("Projects by Language")
        splitter.addWidget(self.language_bar_chart)
        
        # Language details (will be populated later)
        self.language_details = self.create_details_table("Language Details")
        splitter.addWidget(self.language_details)
        
        splitter.setSizes([400, 300])
        layout.addWidget(splitter)
        
        self.tab_widget.addTab(tab, "Languages")
    ...
    def create_category_tab(self):
        ...
        # Category chart
        self.category_bar_chart = self.create_bar_chart("Projects by Category")
        splitter.addWidget(self.category_bar_chart)
        
        # Category details (will be populated later)
        self.category_details = self.create_details_table("Category Details")
    ...
    def create_tags_tab(self):
        ...
        # Tag usage chart
        self.tags_chart = self.create_bar_chart("Most Used Tags")
        splitter.addWidget(self.tags_chart)
        
        # Tag details (will be populated later)
        self.tags_details = self.create_details_table("Tag Details")
    ...
    def create_activity_tab(self):
        ...
        self.activity_chart = self.create_line_chart("Project Activity (Last 30 Days)")
        splitter.addWidget(self.activity_chart)
        
        # Top recent projects
        self.recent_projects_chart = self.create_bar_chart("Most Accessed Projects")
    ...
    def create_size_tab(self):
        ...
        self.size_dist_chart = self.create_bar_chart("Project Size Distribution")
        splitter.addWidget(self.size_dist_chart)
        
        # Largest projects table
        self.largest_projects = self.create_details_table("Largest Projects")
        splitter.addWidget(self.largest_projects)
        
        splitter.setSizes([400, 300])
        layout.addWidget(splitter)
        
        self.tab_widget.addTab(tab, "Size")
    ...
    def create_details_table(self, title: str) -> QFrame:
        ...
        placeholder = QLabel("Loading data...")
    ...
    def load_statistics(self):
        ...
        try:
            ...
            self.status_label.setText(f"Statistics loaded - {len(projects)} projects analyzed")
            
        except Exception as e:
            QMessageBox.warning(self, "Error", f"Could not load statistics: {str(e)}")
            self.status_label.setText("Error loading statistics")
    ...
    def update_line_chart(self, canvas: FigureCanvas, data: Dict[str, int]):
        ...
        if not data or all(v == 0 for v in data.values()):
            ...
            ax.set_ylabel('Projects Accessed')
            ax.set_xlabel('Date')
    ...
    def export_report(self):
        ...
        try:
            ...
            file_path, _ = QFileDialog.getSaveFileName(
                ...
                "project_statistics_report.html", 
            ...
            QMessageBox.warning(self, "Error", f"Could not export report: {str(e)}")
    ...
    def export_html_report(self, file_path: str):
        ...
        <html>
        <head>
            <title>Project Statistics Report</title>
            <style>
...
def show_dashboard(scanner: ProjectScanner, parent=None):
    """Show the dashboard dialog."""
    dialog = DashboardDialog(scanner, parent)
