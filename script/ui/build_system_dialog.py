...
class BuildSystemTab(QWidget):
    ...
    def init_ui(self):
        ...
        info_group = QGroupBox(get_text('build_system.info', 'Build System Information', self.lang))
        info_layout = QFormLayout()
        
        # Build system type
        self.type_label = QLabel(self.build_system_info.get('type', 'Unknown'))
        info_layout.addRow(get_text('build_system.type', 'Type:', self.lang), self.type_label)
        
        # Description
        self.desc_label = QLabel(self.build_system_info.get('description', 'No description'))
        self.desc_label.setWordWrap(True)
        info_layout.addRow(get_text('build_system.description', 'Description:', self.lang), self.desc_label)
        ...
        info_layout.addRow(get_text('build_system.files', 'Files:', self.lang), self.files_text)
        
        info_group.setLayout(info_layout)
        layout.addWidget(info_group)
        
        # Build commands group
        commands_group = QGroupBox(get_text('build_system.commands', 'Build Commands', self.lang))
        commands_layout = QVBoxLayout()
        
        self.commands_table = QTableWidget()
        self.commands_table.setColumnCount(2)
        self.commands_table.setHorizontalHeaderLabels([
            get_text('build_system.command', 'Command', self.lang),
            get_text('build_system.description', 'Description', self.lang)
        ])
        ...
        self.refresh_btn = QPushButton(get_text('build_system.refresh', 'Refresh', self.lang))
        self.refresh_btn.clicked.connect(self.refresh_build_system)
        button_layout.addWidget(self.refresh_btn)
        
        self.copy_command_btn = QPushButton(get_text('build_system.copy_command', 'Copy Command', self.lang))
    ...
    def refresh_build_system(self):
        ...
        try:
            ...
            QMessageBox.information(self, get_text('build_system.refresh_success', 'Success', self.lang),
                                  get_text('build_system.refreshed', 'Build system information refreshed', self.lang))
        except Exception as e:
            QMessageBox.warning(self, get_text('build_system.refresh_error', 'Error', self.lang),
    ...
    def copy_selected_command(self):
        ...
        if current_row >= 0:
            ...
            if command_item:
                ...
                QMessageBox.information(self, get_text('build_system.copied', 'Copied', self.lang),
                                      get_text('build_system.command_copied', 'Command copied to clipboard', self.lang))
...
class DependenciesTab(QWidget):
    ...
    def init_ui(self):
        ...
        self.dependencies_tree.setHeaderLabels([
            get_text('dependencies.package', 'Package', self.lang),
            get_text('dependencies.version', 'Version', self.lang),
            get_text('dependencies.type', 'Type', self.lang),
            get_text('dependencies.status', 'Status', self.lang)
        ])
        ...
        self.refresh_btn = QPushButton(get_text('dependencies.refresh', 'Refresh', self.lang))
        self.refresh_btn.clicked.connect(self.load_dependencies)
        button_layout.addWidget(self.refresh_btn)
        
        self.update_btn = QPushButton(get_text('dependencies.update', 'Update Selected', self.lang))
        self.update_btn.clicked.connect(self.update_selected_dependencies)
        button_layout.addWidget(self.update_btn)
        
        self.install_btn = QPushButton(get_text('dependencies.install', 'Install New', self.lang))
        self.install_btn.clicked.connect(self.install_new_dependency)
        button_layout.addWidget(self.install_btn)
        
        self.remove_btn = QPushButton(get_text('dependencies.remove', 'Remove Selected', self.lang))
    ...
    def on_analysis_error(self, error: str):
        ...
        QMessageBox.warning(self, get_text('dependencies.analysis_error', 'Analysis Error', self.lang),
    ...
    def update_statistics(self, analysis: Dict[str, Any]):
        ...
        stats_text = get_text('dependencies.stats', 
                            'Total: {total} | Outdated: {outdated} | Vulnerabilities: {vuln}', self.lang).format(
    ...
    def update_selected_dependencies(self):
        ...
        if not selected_items:
            QMessageBox.warning(self, get_text('dependencies.no_selection', 'No Selection', self.lang),
                              get_text('dependencies.select_deps', 'Please select dependencies to update', self.lang))
            return
        ...
        if not package_manager:
            QMessageBox.warning(self, get_text('dependencies.no_pm', 'No Package Manager', self.lang),
                              get_text('dependencies.cannot_determine_pm', 'Cannot determine package manager', self.lang))
            return
        
        # Confirm update
        reply = QMessageBox.question(self, get_text('dependencies.confirm_update', 'Confirm Update', self.lang),
        ...
        if reply == QMessageBox.Yes:
            ...
            if success:
                QMessageBox.information(self, get_text('dependencies.update_success', 'Success', self.lang),
                                      get_text('dependencies.updated', 'Dependencies updated successfully', self.lang))
                self.load_dependencies()  # Refresh
            else:
                QMessageBox.warning(self, get_text('dependencies.update_error', 'Error', self.lang),
                                  get_text('dependencies.update_failed', 'Failed to update dependencies', self.lang))
    
    def install_new_dependency(self):
        """Install a new dependency."""
        # Simple dialog for dependency name input
        dialog = QDialog(self)
        dialog.setWindowTitle(get_text('dependencies.install_dep', 'Install Dependency', self.lang))
        layout = QFormLayout()
        ...
        layout.addRow(get_text('dependencies.name', 'Name:'), name_input)
        layout.addRow(get_text('dependencies.version', 'Version (optional):'), version_input)
        layout.addRow(get_text('dependencies.package_manager', 'Package Manager:'), pm_combo)
        ...
        if dialog.exec() == QDialog.Accepted:
            ...
            if not name:
                QMessageBox.warning(self, get_text('dependencies.no_name', 'No Name', self.lang),
                                  get_text('dependencies.enter_name', 'Please enter dependency name', self.lang))
                return
            ...
            if success:
                QMessageBox.information(self, get_text('dependencies.install_success', 'Success', self.lang),
                                      get_text('dependencies.installed', 'Dependency installed successfully', self.lang))
                self.load_dependencies()  # Refresh
            else:
                QMessageBox.warning(self, get_text('dependencies.install_error', 'Error', self.lang),
                                  get_text('dependencies.install_failed', 'Failed to install dependency', self.lang))
    ...
    def remove_selected_dependencies(self):
        ...
        if not selected_items:
            QMessageBox.warning(self, get_text('dependencies.no_selection', 'No Selection', self.lang),
                              get_text('dependencies.select_deps', 'Please select dependencies to remove', self.lang))
            return
        ...
        # Confirm removal
        reply = QMessageBox.question(self, get_text('dependencies.confirm_remove', 'Confirm Remove', self.lang),
        ...
        if reply == QMessageBox.Yes:
            ...
            if success:
                QMessageBox.information(self, get_text('dependencies.remove_success', 'Success', self.lang),
                                      get_text('dependencies.removed', 'Dependencies removed successfully', self.lang))
                self.load_dependencies()  # Refresh
            else:
                QMessageBox.warning(self, get_text('dependencies.remove_error', 'Error', self.lang),
                                  get_text('dependencies.remove_failed', 'Failed to remove dependencies', self.lang))
...
class BackupTab(QWidget):
    ...
    def init_ui(self):
        ...
        config_group = QGroupBox(get_text('backup.configuration', 'Backup Configuration', self.lang))
        config_layout = QFormLayout()
        
        self.auto_backup_cb = QCheckBox()
        self.auto_backup_cb.setChecked(self.backup_system.config.get('auto_backup', True))
        config_layout.addRow(get_text('backup.auto_backup', 'Automatic Backup:', self.lang), self.auto_backup_cb)
        ...
        self.interval_spin.setSuffix(get_text('backup.hours', ' hours', self.lang))
        config_layout.addRow(get_text('backup.interval', 'Backup Interval:', self.lang), self.interval_spin)
        
        self.max_backups_spin = QSpinBox()
        self.max_backups_spin.setRange(1, 100)
        self.max_backups_spin.setValue(self.backup_system.config.get('max_backups', 10))
        config_layout.addRow(get_text('backup.max_backups', 'Maximum Backups:', self.lang), self.max_backups_spin)
        
        self.compress_cb = QCheckBox()
        self.compress_cb.setChecked(self.backup_system.config.get('compress_backups', True))
        config_layout.addRow(get_text('backup.compress', 'Compress Backups:', self.lang), self.compress_cb)
        
        config_group.setLayout(config_layout)
        layout.addWidget(config_group)
        
        # Backup list
        list_group = QGroupBox(get_text('backup.existing_backups', 'Existing Backups', self.lang))
        list_layout = QVBoxLayout()
        
        self.backup_table = QTableWidget()
        self.backup_table.setColumnCount(4)
        self.backup_table.setHorizontalHeaderLabels([
            get_text('backup.name', 'Name', self.lang),
            get_text('backup.date', 'Date', self.lang),
            get_text('backup.size', 'Size', self.lang),
            get_text('backup.compressed', 'Compressed', self.lang)
        ])
        ...
        self.create_btn = QPushButton(get_text('backup.create', 'Create Backup', self.lang))
        self.create_btn.clicked.connect(self.create_backup)
        button_layout.addWidget(self.create_btn)
        
        self.restore_btn = QPushButton(get_text('backup.restore', 'Restore Selected', self.lang))
        self.restore_btn.clicked.connect(self.restore_selected_backup)
        button_layout.addWidget(self.restore_btn)
        
        self.delete_btn = QPushButton(get_text('backup.delete', 'Delete Selected', self.lang))
        self.delete_btn.clicked.connect(self.delete_selected_backup)
        button_layout.addWidget(self.delete_btn)
        
        self.save_config_btn = QPushButton(get_text('backup.save_config', 'Save Config', self.lang))
    ...
    def create_backup(self):
        ...
        try:
            ...
            if backup_file:
                QMessageBox.information(self, get_text('backup.create_success', 'Success', self.lang),
                                      get_text('backup.created', 'Backup created successfully', self.lang))
                self.load_backup_list()  # Refresh
            else:
                QMessageBox.warning(self, get_text('backup.create_error', 'Error', self.lang),
                                  get_text('backup.create_failed', 'Failed to create backup', self.lang))
        except Exception as e:
            QMessageBox.warning(self, get_text('backup.create_error', 'Error', self.lang),
    ...
    def restore_selected_backup(self):
        ...
        if current_row < 0:
            QMessageBox.warning(self, get_text('backup.no_selection', 'No Selection', self.lang),
                              get_text('backup.select_backup', 'Please select a backup to restore', self.lang))
            return
        
        backup_name = self.backup_table.item(current_row, 0).text()
        
        # Confirm restore
        reply = QMessageBox.question(self, get_text('backup.confirm_restore', 'Confirm Restore', self.lang),
                                   f"Restore backup '{backup_name}'? This will overwrite current data.",
                                   QMessageBox.Yes | QMessageBox.No)
        
        if reply == QMessageBox.Yes:
            # Get restore path
            restore_path = QFileDialog.getExistingDirectory(self, get_text('backup.select_restore_path', 'Select Restore Path', self.lang))
            if restore_path:
                success = self.backup_system.restore_backup(backup_name, restore_path)
                if success:
                    QMessageBox.information(self, get_text('backup.restore_success', 'Success', self.lang),
                                          get_text('backup.restored', 'Backup restored successfully', self.lang))
                else:
                    QMessageBox.warning(self, get_text('backup.restore_error', 'Error', self.lang),
                                      get_text('backup.restore_failed', 'Failed to restore backup', self.lang))
    ...
    def delete_selected_backup(self):
        ...
        if current_row < 0:
            QMessageBox.warning(self, get_text('backup.no_selection', 'No Selection', self.lang),
                              get_text('backup.select_backup', 'Please select a backup to delete', self.lang))
            return
        
        backup_name = self.backup_table.item(current_row, 0).text()
        
        # Confirm deletion
        reply = QMessageBox.question(self, get_text('backup.confirm_delete', 'Confirm Delete', self.lang),
        ...
        if reply == QMessageBox.Yes:
            ...
            if success:
                QMessageBox.information(self, get_text('backup.delete_success', 'Success', self.lang),
                                      get_text('backup.deleted', 'Backup deleted successfully', self.lang))
                self.load_backup_list()  # Refresh
            else:
                QMessageBox.warning(self, get_text('backup.delete_error', 'Error', self.lang),
                                  get_text('backup.delete_failed', 'Failed to delete backup', self.lang))
    ...
    def save_config(self):
        ...
        if success:
            QMessageBox.information(self, get_text('backup.config_saved', 'Success', self.lang),
                                  get_text('backup.config_updated', 'Backup configuration updated', self.lang))
        else:
            QMessageBox.warning(self, get_text('backup.config_error', 'Error', self.lang),
                              get_text('backup.config_failed', 'Failed to update backup configuration', self.lang))
...
class BuildSystemDialog(QDialog):
    ...
    def init_ui(self):
        self.setWindowTitle(get_text('build_system_dialog.title', 'Build System & Dependencies', self.lang))
        self.setModal(True)
        self.resize(800, 600)
        ...
        self.tab_widget.addTab(self.build_system_tab, get_text('build_system_dialog.build_tab', 'Build System', self.lang))
        
        # Dependencies tab
        self.dependencies_tab = DependenciesTab(self.project_path)
        self.tab_widget.addTab(self.dependencies_tab, get_text('build_system_dialog.deps_tab', 'Dependencies', self.lang))
        
        # Backup tab
        self.backup_tab = BackupTab(self.project_path, self.lang)
        self.tab_widget.addTab(self.backup_tab, get_text('build_system_dialog.backup_tab', 'Backup', self.lang))
