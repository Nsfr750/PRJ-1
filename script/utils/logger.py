...
class PRJLogger:
    """Advanced logger for PRJ-1 application with enhanced features."""
    
    def __init__(self, name: str = 'prj1', log_level: int = logging.INFO):
        self.name = name
    ...
    def _setup_file_handlers(self):
        ...
        main_log_file = self.config['logs_dir'] / f'prj_{current_date}.log'
        ...
        error_log_file = self.config['logs_dir'] / f'prj_errors_{current_date}.log'
        ...
        if self.config['json_logging']:
            json_log_file = self.config['logs_dir'] / f'prj_json_{current_date}.log'
...
class LoggerManager:
    ...
    @classmethod
    def get_logger(cls, name: str = 'prj1', **kwargs) -> PRJLogger:
        """Get or create a logger instance."""
        if name not in cls._instances:
            cls._instances[name] = PRJLogger(name)
...
def setup_logger(name: str = 'prj1', log_level: int = logging.INFO, **kwargs) -> PRJLogger:
    """Setup and return a configured logger instance."""
    return LoggerManager.get_logger(name, log_level=log_level, **kwargs)


def get_logger(name: str = None) -> PRJLogger:
    """Get a logger instance."""
    if name:
        return LoggerManager.get_logger(f'prj.{name}')
    return LoggerManager.get_logger('prj1')
...
def add_custom_methods(logger_instance: PRJLogger):
